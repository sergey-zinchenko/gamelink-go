...
SELECT JSON_OBJECT(
	...
	"rank"		,   rank,
	"country"	,	i.country,
    "leaderboard", 	leaderboard) as flb FROM (SELECT * FROM leader_board%[1]d u WHERE u.id=?) as i,
				(SELECT (CAST(IFNULL(CONCAT('[', GROUP_CONCAT(DISTINCT CONCAT('{',
                                                            '"id":',     	l.id,         ',',
                                                            '"nickname":',  IFNULL(JSON_QUOTE(l.nickname),JSON_QUOTE(l.name)), ',',
                                                            '"score":',   	l.score, ',',
                                                            ...
                                                            '}')), ']'),"[]") AS JSON)) AS leaderboard 
FROM 
(SELECT j.id, j.nickname,j.name, j.country, j.score FROM leader_board%[1]d j WHERE
(j.id IN (SELECT f.user_id2 FROM friends f WHERE f.user_id1 = ?)) AND j.score > 0) as l) as c,
(SELECT i.*, COUNT(*) + 1 as rank FROM leader_board%[1]d  i, leader_board%[1]d  j
WHERE i.id = ? AND j.id IN (SELECT f.user_id2 FROM friends f WHERE f.user_id1 = ?) AND j.score > i.score) as rank`
)
